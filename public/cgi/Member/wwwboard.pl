...
$ENV{SCRIPT_FILENAME} =~ m#^(.*?)/([^/]+)/webdocs/cgi/(.*?)/wwwboard.pl$#;
$HOME    = "$1/$2";
$FQDN    = $2;
$SUBDIR  = $3;
...
$basedir = "$HOME/webdocs/$SUBDIR";
$baseurl = "http://$FQDN/$SUBDIR";
$cgi_url = "http://$FQDN/cgi/$SUBDIR/wwwboard.pl";
$header = "$HOME/webdocs/$SUBDIR/wwwbhead.shtml";
$footer = "$HOME/webdocs/$SUBDIR/wwwbfoot.shtml";
$mesgdir = "messages";
$datafile = "data.txt";
$mesgfile = "wwwboard.shtml";
$faqfile = "faq.shtml";
...
sub new_file {
   ...
   if( ! open(NEWFILE,">$basedir/$mesgdir/$num\.$ext") ) {
       print STDERR "cannot open $basedir/$mesgdir/$num\n";
       die $!;
   }
&print_NEWFILE_header;
...
sub print_header  {
  open(FILE, $header)  || warn "Can't open header: $!\n";
  while (<FILE>)  {
    print $_;
  }
}
...
sub print_footer  {
  open(FILE, $footer)  || warn "Can't open footer: $!\n";
  while (<FILE>)  {
    print $_;
  }
}
