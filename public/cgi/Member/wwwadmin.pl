...
open( STDERR, ">stderr.wwwadmin.txt" );  # debug

$ENV{SCRIPT_FILENAME} =~ m#^(.*?)/([^/]+)/webdocs/cgi/(.*?)/wwwadmin.pl$#;
$HOME    = "$1/$2";
$FQDN    = $2;
$SUBDIR  = $3;
...
$cgi_url = "http://$FQDN/cgi/$SUBDIR/wwwadmin.pl";

$header = "$HOME/www/wwwboard/wwwbhead.shtml";
$footer = "$HOME/www/wwwboard/wwwbfoot.shtml";


if( 0 ) {
    print STDERR "SCRIPT_FILENAME=$ENV{SCRIPT_FILENAME}\n";
    ...
    print STDERR "cgi_url=$cgi_url\n";
    print STDERR "header=$header\n";
    print STDERR "footer=$footer\n";
}
...
###########################################################################
   ...
   print "<li><a href=\"$cgi_url\?remove_by_num\">Remove Files by Message Number</a>\n";
